...
if (currentWishesEl) {
  let getStartPatch2 = function() {
    ...
    for (let i = 0; i <= total; i++) {
      let labelStr;
      if (i === 0) labelStr = `Current Patch (${patchVersionAt2(0)})`;
      else if (i === 1) labelStr = `Next Patch (${patchVersionAt2(1)})`;
      else if (i === 2) labelStr = `2 Patches Later (${patchVersionAt2(2)})`;
      else labelStr = `${i} Patches Later (${patchVersionAt2(i)})`;
      container.innerHTML += `
    ...
    function getIncomeCeiling(item) {
      ...
      for (let i = 0; i < item.targetPatch; i++) {
        income += getPatchIncome2(i);
      }
      const patchIncome = getPatchIncome2(item.targetPatch);
    ...
    function runScenario(losePattern) {
      ...
      const enabledItems = expandPipeline2(priorityPipeline.filter((x) => x.enabled !== false));
      enabledItems.forEach((item, idx) => {
        ...
        const patchLabel = patchVersionAt2(item.targetPatch);
    ...
    function renderRow(row) {
      const rowIcon = avatarBadgeHtml2(row.icon, elementIconPath2(row.element), 44, 17);
      if (row.type === "skip") {
    ...
    const ep = expandPipeline2(enabledPipeline);
    const nep = ep.length;
    ...
    function solveWithReservation(rawPattern) {
      ...
      let resolved = resolvePattern2(raw, ep);
      for (let guard = 0; guard < nep + 1; guard++) {
        ...
        if (!changed) break;
        resolved = resolvePattern2(raw, ep);
      }
      return resolved;
    }
    ...
    const odds = computeScenarioOdds2(ep, worstPattern);
    outputSpace.innerHTML += renderScenarioSummary2(results, ep, odds);
    ...
    return expanded;
  }, resolvePattern2 = function(rawPattern, expandedPipeline) {
    return applyCaptureRadiance2(rawPattern, expandedPipeline);
  }, itemWinProb2 = function(item) {
    return item.type === "weapon" ? 0.75 : 0.55;
  }, applyCaptureRadiance2 = function(pattern, pipeline) {
  ...
  }, renderScenarioSummary2 = function(results, ep, odds) {
            ...
            <div class="scenario-summary-card">
                <div class="scen-sum-stats">
                    ...
                    <div class="scen-sum-stat">
                        ...
                        <span class="scen-sum-stat-val" style="color:var(--warning, #d9a441)">${formatChance2(odds.mixedPct, false)}</span>
                    </div>
                    <div class="scen-sum-stat">
    ...
    const blob = new Blob([JSON.stringify(buildState2(), null, 2)], { type: "application/json" });
  ...
  [hasWelkinEl, hasBPEl].forEach((el) => el.addEventListener("change", calculateForecast2));
  ...
  [hasWelkinEl, hasBPEl].forEach((el) => el.addEventListener("change", saveState2));
